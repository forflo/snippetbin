...
struct settings *parse_args(int argc, char **argv){
	...
	while((c = getopt_long(argc, argv, OPTSTRING, long_options, &oi)) != -1){
		switch (c) {
			case 'h': s->help = TRUE; break;
			case 'v': s->version = TRUE; break;
			case 'c': s->create = TRUE; break;
			case 'f': s->file_input = TRUE; break;
			case 'd': s->destroy = TRUE; break;
			case 'l': s->size = atoi(optarg); break;
			case 'k': s->key = atoi(optarg); break;
			case 'p': 
				...
				break;
			case 'g': s->get = TRUE; break;
			case 's': s->set = TRUE; break;
...
int main(int argc, char **argv){
	...
	/* if no permission is given, the standard is used, which is 0600 */
		...
		/* if no data is provided (neither through args->content nor through STDIN) 
			everything ist finde. Just the ASCII null will be written */
